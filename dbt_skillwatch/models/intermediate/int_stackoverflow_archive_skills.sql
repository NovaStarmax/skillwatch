with staged as (
    ...
    select * from {{ ref('stg_stackoverflow_archive') }}

),

unpivoted as (

    select response_id, survey_year, salary_usd, trim(lower(skill_raw)) as skill_raw
    ...
    select response_id, survey_year, salary_usd, trim(lower(skill_raw)) as skill_raw
    from staged, unnest(string_to_array(misc_tech_have_worked_with, ';')) as skill_raw

    union all

    select response_id, survey_year, salary_usd, trim(lower(skill_raw)) as skill_raw
    from staged, unnest(string_to_array(tools_tech_have_worked_with, ';')) as skill_raw
...
from matched
group by canonical_skill, survey_year
